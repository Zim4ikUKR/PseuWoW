...
void CM2Mesh::LinkChildMeshes (IAnimatedMeshSceneNode *ParentMeshNode, ISceneManager *smgr, core::array<IBoneSceneNode*> &JointChildSceneNodes) // Use ((scene::CM2Mesh*)(m))->createJoints before using linkchildmeshes
{
	//std::string UI_ChildMeshName = core::stringc(smgr->getMeshCache()->getMeshName(UI_ParentMeshNode->getMesh())).c_str() ; // get the parent mesh's name
	// ToDo:: atatch nodes for all submeshes in all skins ignoring duplicates
	ISceneNode *skybox = smgr->addSkyBoxSceneNode(0,0,0,0,0,0);
	ISceneNode *scene = smgr->addEmptySceneNode();
	ISceneNode *last = scene;
	for(u16 s=0; s < Skins[CM2Mesh::SkinID].Submeshes.size(); s++)
	{
		...
		// set the new node as a child of it's parent (It is also possible to atach the child to the bone it shares with the parent)
		//last->addChild(ChildMeshNode);
		if (Skins[CM2Mesh::SkinID].Submeshes[s].Radius > 70){
			skybox->addChild(ChildMeshNode);
			smgr->registerNodeForRendering(ChildMeshNode, ESNRP_NONE);    // Clear default passes
			smgr->registerNodeForRendering(ChildMeshNode, ESNRP_SKY_BOX);
		}
		else{
			//last->addChild(ChildMeshNode);
		//ParentMeshNode->addChild(ChildMeshNode);  // by default I am ataching the childeren to the empty mesh generated by the loader
		//JointChildSceneNodes[Skins[CM2Mesh::SkinID].Submeshes[s].RootBone]->addChild(ChildMeshNode);
			ChildMeshNode->setParent(JointChildSceneNodes[Skins[SkinID].Submeshes[s].RootBone]);
		//ChildMeshNode->updateAbsolutePosition();
		}
...
void CM2Mesh::findExtremes() 
{
	...
	for (u32 s=0; s>getMeshBufferCount()-1; s++) // Loop through submeshes
	{
		...
		if (S->getIndexCount() <= 8){count = S->getIndexCount();}// if there are less than 8 vertices we will use them all 
		else if (S->getIndexCount() < 32){count = 8;} // simple submeshes use 8 extremes
		else {count = 10;} // complex ones get 10

		core::list<Group_of_Points> boxes;
		...
		(*boxes.begin()).computeBBox(S);

		// Remember the geometrical center of the submesh
		core::vector3df center = ((*boxes.begin()).Box.MaxEdge + (*boxes.begin()).Box.MinEdge) * 0.5;
		...
		while (boxes.size () < count)
		{
			...
			Group_of_Points *split_box = NULL;
			float split_volume = -1;
			for (core::list<Group_of_Points>::Iterator b = boxes.begin ();
					b != boxes.end (); ++b)
			{
				if (b->empty ())
					continue;
			...
			int split_axis = 0;
			float split_length = split_box->Box.MaxEdge.X - split_box->Box.MinEdge.X;
			for (int i = 1; i < 3; i++) // i indexes into a vector3df  1=x 2=y 3=z
			{
				float l = split_box->getValueByAxis_FromVector(i, split_box->Box.MaxEdge) - split_box->getValueByAxis_FromVector(i, split_box->Box.MinEdge);
				if (l > split_length)
				{
					split_length = l;
					split_axis = i;
			...
			boxes.push_back (split_box->split (split_axis, S));
		}

		// Each box contains idices to a group of points from the submesh. Fevery group find the best extreme point.  It will be the vertex that is most
		// distant from the center of the current submesh and from the other extremes.
		for (core::list<Group_of_Points>::ConstIterator b = boxes.begin (); b != boxes.end (); ++b)
		{
			float rating = 0;
			u32 best_vertex;

			// Iterate over the groups of points we subdivided the current submesh into.
			for (core::list<u32>::ConstIterator i = b->mIndices.begin (); i != b->mIndices.end (); ++i)
			{
				...
				{
					rating = r;
					best_vertex = *i; // store index to current vertex
				}
			}
